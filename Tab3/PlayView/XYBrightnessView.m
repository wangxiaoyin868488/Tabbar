...
@implementation XYBrightnessView

+ (instancetype)sharedBrightnesView {
    static XYBrightnessView *instance;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [[XYBrightnessView alloc]init];
...
- (instancetype)init {
    if (self = [super init]) {
        self.frame = CGRectMake(kwidth * 0.5, kheight * 0.5, 155, 155);
    ...
    if (self.orientationDidChange) {
        [UIView animateWithDuration:0.25 animations:^{
            if ([UIDevice currentDevice].orientation == UIDeviceOrientationPortrait
                ...
                self.center = CGPointMake(kwidth * 0.5, (kheight - 10) * 0.5);
            } else {

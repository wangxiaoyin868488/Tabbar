...
static const CGFloat XYPlayerControlBarAutoFadeOutTimeInterval = 0.5f;
...
@interface XYPlayerView () <UIGestureRecognizerDelegate,UIAlertViewDelegate>
...
{
    return [[NSBundle mainBundle] loadNibNamed:@"XYPlayerView" owner:nil options:nil].lastObject;
}
...
- (void)awakeFromNib
{
    ...
    self.state = XYPlayerStateStopped;
...
- (void)setVideoURL:(NSURL *)videoURL
{
    ...
    self.maskView = [XYPlayerMaskView setupPlayerMaskView];
    ...
    // 本地文件不设置XYPlayerStateBuffering状态
...
- (void)autoFadeOutControlBar
{
    ...
    [self performSelector:@selector(hideMaskView) withObject:nil afterDelay:XYPlayerAnimationTimeInterval];
...
{
    if (state != XYPlayerStateBuffering) {
